(function() {
  var pagseguro,
      uriConfig   = require('./../config/config.uri'),
      req         = require('request'),
      jsonParse         = require('jstoxml'),
  ...
  pagseguro = (function() {
    function pagseguro(configObj) {
      ...
      this.xsd = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>';
      return this;
    }
    ...
    pagseguro.prototype.extraAmount = function(extraAmount) {
      this.obj.extraAmount = extraAmount;
      return this;
    };
    ...
    pagseguro.prototype.receiverEmail = function (email) {
      this.obj.receiver = {};
      this.obj.receiver.email = email;
      return this;
    };
    ...
    pagseguro.prototype.send = function(callback) {
      ...
      switch (this.mode) {
        case 'payment':
          options.uri = uriConfig[this.mode].checkout(this.configObj);
          options.body = this.xsd + jsonParse.toXML({
            checkout: this.obj
          });
          break;
        case 'subscription':
      ...
      return req(options, function(err, res, body) {
        if (err) {
          ...
          return callback(null, xmlParse.toJson(body, xmlParseOptions));
        }
      });
    };

    pagseguro.prototype.notificationConsult =  function(notificationCode, callback) {
      var options = {
        method: 'GET',
        headers: {
          'Content-Type': 'application/xml; charset=UTF-8'
      ...
      options.uri = uriConfig[this.mode].notificationConsult(notificationCode, this.configObj);
    ...
    pagseguro.prototype.transactionConsult =  function(transactionCode, callback) {
      var options = {
        method: 'GET',
        headers: {
          'Content-Type': 'application/xml; charset=UTF-8'
      ...
      options.uri = uriConfig[this.mode].transactionConsult(transactionCode, this.configObj);
    ...
    pagseguro.prototype.paymentUri = function(paymentCode) {
      return uriConfig[this.mode].payment(paymentCode);
    };

    pagseguro.prototype.statusByCode = function(code) {
      var status = [
        {code: 1, display: 'Aguardando pagamento'},
        {code: 2, display: 'Em análise'},
        {code: 3, display: 'Paga'},
        {code: 4, display: 'Disponível'},
        {code: 5, display: 'Em disputa'},
        {code: 6, display: 'Devolvida'},
        {code: 7, display: 'Cancelada'},
        {code: 8, display: 'Debitado'},
        {code: 9, display: 'Retenção temporária'}
      ];
      var state = status.filter(function(state) {
        return state.code === parseInt(code);
      });
      console.log(state);
      return state[0];
    };

    pagseguro.prototype.statusPaymentApproved = function() {
      return 3;
    };
